...
class BinaryTreeNode {
 public:
  ...
  void visit_in_order(list<int>& visited) {
    if (left) left->visit_in_order(visited);
    visited.push_back(data);
    if (right) right->visit_in_order(visited);
  }

  BinaryTreeNode* search(int n) {
    if (data == n) return this;

    if (n < data && left) return left->search(n);
    if (n > data && right) return right->search(n);

    return NULL;
  }

  bool contains(int n) { return (search(n) != NULL); }

  // Must be called on the root node so the full tree is searched.
  void remove(int n) { removeNode(n); }
